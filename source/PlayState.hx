...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'null':
				dialogue = CoolUtil.coolTextFile(Paths.txt('null/nullDialogue'));
		}
		...
		case 'light':
			{
					...
					defaultCamZoom = 0.65;
					curStage = 'light';
					light = new FlxSprite(-100, -60).loadGraphic(Paths.image('bg', 'ink'));
					light.antialiasing = true;
					light.scrollFactor.set(0.9, 0.9);
					light.setGraphicSize(Std.int(light.width * 2.0));
					light.active = false;
					add(light);

					paper = new FlxSprite(0 ,-300);
					paper.frames = Paths.getSparrowAtlas('paper', 'ink');
					paper.animation.addByPrefix('idle', 'paper00', 24);
					paper.animation.play('idle');
					paper.setGraphicSize(Std.int(paper.width * 1.0));
			        paper.updateHitbox();
					...
					error = new FlxSprite(-840, 300);
					error.frames = Paths.getSparrowAtlas('error sans','ink');
					error.animation.addByPrefix('idle', 'error sans0', 24);
					error.animation.play('idle');
					error.setGraphicSize(Std.int(error.width * 0.3));
			        error.updateHitbox();
	...
	override public function update(elapsed:Float)
	{
		...
		if (SONG.song.toLowerCase() == 'null' && IsNoteSpinning){
			var thisX:Float =  Math.sin(SpinAmount * (SpinAmount / 2)) * 70;
			var thisY:Float =  Math.sin(SpinAmount * (SpinAmount)) * 70;
		...
		else
		{
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					{
						case "null":
								FlxG.switchState(new VideoState("assets/videos/cut2.webm",new StoryMenuState()));
						/*case "evening":
	...
	private function popUpScore(daNote:Note):Void
        ...
        {
            sploosh.loadGraphic(Paths.image('noteSplashes-pixels', 'shared'), true, 50, 50);
		...
		if (SONG.song.toLowerCase() == 'null')
			switch (curStep)
			{
				        ...
				        case 164:
								...
								gf.alpha = 0;
								remove(boyfriend);
								boyfriend.frames = Paths.getSparrowAtlas('characters/BOYFRIENDwhite','shared');
								...
								boyfriend.animation.addByPrefix('firstDeath', "BF dies", 24, false);
								boyfriend.animation.addByPrefix('deathLoop', "BF Dead Loop", 24, true);
								boyfriend.animation.addByPrefix('deathConfirm', "BF Dead confirm", 24, false);
								...
								boyfriend.addOffset('firstDeath', 37, 11);
								boyfriend.addOffset('deathLoop', 37, 5);
								boyfriend.addOffset('deathConfirm', 37, 69);
